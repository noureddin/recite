...
const whole = new Set(['ar_ma3any', 'ar_muyassar'])
const tafsir = {}
...
function tv (i) {
  ...
  const head = `<p>${title} للآية ${filter_aaya_input(i - sura_offset[s])} من سورة ${suar_name[s]}</p><p id="tafsirnote">(يمكن تغيير التفسير من «الخيارات» في أعلى الصفحة بعد إغلاق التفسير)</p><p class="aya">${aya}</p><hr>`
  // loading screen
  el_tv.innerHTML = head + spinner
  // get the tafsir
  get_tafsir(name, i, (content) => { el_tv.innerHTML = head + '<div id="tvtxt">'+content+'</div>' })
}
